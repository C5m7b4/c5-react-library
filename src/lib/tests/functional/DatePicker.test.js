...
import { getTime, nextDate } from "../../DatePicker/time";
import eventTrigger from "../event_helper";
...
describe("DatePicker.js", () => {
  describe("Lifecycle", () => {
    it("should update value of state when parent component value of props update", () => {
      const datePicker = mount(<DatePicker {...DEFAULT_PROPS} />);
      datePicker.setProps({ value: new Date(2016, 8, 15) });
      expect(datePicker.state("value").getTime()).to.equals(
        new Date(2016, 8, 15).getTime()
      );
    });
  });
  ...
  describe("logic", () => {
    ...
    beforeEach(function() {
      datePicker = mount(<DatePicker {...DEFAULT_PROPS} />);
...
describe("渲染正确的DatepicketItem子组件", () => {
  ...
  const datePicker = () => {
    if (!mountedDatepicker) {
      mountedDatepicker = mount(<DatePicker {...props} />);
    }
...
describe("测试step", () => {
  ...
  it("当datesteps等于[5, 5, 5], dateFormart等于[hh, mm, ss], 当前时间为8:20:57，向上滑动秒，分钟应该为23", () => {
    ...
    return delay(250).then(() => {
      expect(mountedDatepicker.state("value").getTime()).to.equals(
        new Date(2016, 8, 16, 8, 23, 2).getTime()
      );
    });
  });
  ...
  it("当datesteps等于[5, 5, 5], dateFormart等于[hh, mm, ss], 当前时间为8:20:57，向上滑动秒，最大时间是8:20:59, 分钟应该为22", () => {
    ...
    return delay(250).then(() => {
      expect(mountedDatepicker.state("value").getTime()).to.equals(
        new Date(2016, 8, 16, 8, 22, 57).getTime()
      );
    });
  });
});
...
describe("测试showCaption属性", () => {
  ...
  it("显示说明", () => {
    ...
    const wrapper = datePicker();
    expect(wrapper.find(".datepicker-caption").text()).to.be.equal(
      "MonthDayHour"
    );
  });
});
